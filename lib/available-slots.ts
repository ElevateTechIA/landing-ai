import { checkAvailability } from './google-calendar';
 ...
 */
export async function getAvailableSlots(params: GetAvailableSlotsParams = {}): Promise<GetAvailableSlotsResult> {
  try {
    const {
      desiredDate,
      count = 5,
    ...
    console.log('[GET_AVAILABLE_SLOTS] Request:', {
      desiredDate,
      count,
      language,
      timeRange,
      ...
      for (let hour = startHour; hour < endHour; hour++) {
        for (let minute = 0; minute < 60; minute += 30) {
          ...
          try {
            ...
            console.error('[GET_AVAILABLE_SLOTS] Error checking slot:', {
              slot: slotStart.toISOString(),
              error: error instanceof Error ? error.message : String(error),
            });
            // Continue checking other slots even if one fails
          }
        }
        if (availableSlots.length >= count) break;
      }
    ...
    console.log('[GET_AVAILABLE_SLOTS] Found slots:', availableSlots.length);
